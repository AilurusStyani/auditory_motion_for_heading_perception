function AudioTunnel3DDemo_test
...
WalkDegree = [50 90 130];
walkspeed = 50;
axis_direction = ([cosd(WalkDegree) ; zeros(1,length(WalkDegree)) ; sind(WalkDegree)] * walkspeed)';

nsources = 3; %%%% should be more than 1, currently dont know why
...
for j = 1 : nsources
    ...
    soundname = [sounddir soundfiles(mod(perm(j), length(soundfiles))+1).name];
    ...
    alBufferData( buffers(j), AL.FORMAT_MONO16, mynoise, length(mynoise)*2, freq);

    % Attach our buffer to it: The source will play the buffers sound data.
    alSourceQueueBuffers(sources(j), 1, buffers(j));

    % Switch source to looping playback: It will repeat playing the buffer until
    % its stopped.
    alSourcei(sources(j), AL.LOOPING, AL.TRUE);

    % Set emission volume to 100%, aka a gain of 1.0:
    alSourcef(sources(j), AL.GAIN, 0.9);

    alSourcef(sources(j), AL.CONE_INNER_ANGLE, 30);
    alSourcef(sources(j), AL.CONE_OUTER_ANGLE, 270);
    alSource3f(sources(j), AL.DIRECTION, 0, 0, 1);

    % Set spurces position
    alSource3f(sources(j), AL.POSITION, 0, 0, 20);

    % Sources themselves remain static in space:
    alSource3f(sources(j), AL.VELOCITY, 0, 0, 0);

    % Start playback for these sources:
    alSourcePlayv(nsources, sources);
end

for i = 1 : length(WalkDegree)
    cur_time = tic;
    while toc(cur_time) < 1
