...
% sort data_cum in row by x, BYC 20230223
[~,I] = sort(data_cum(:,1));
data_cum = data_cum(I,:);
...
% now it should works for all rangs of x, by BYC 20230223
xRange = max(data_cum(:,1)) - min(data_cum(:,1));
nlog = ceil(log(xRange) / log(10));
threshold_e = 10.^(nlog-4:nlog);
bias_e = sort([-threshold_e, 0, threshold_e]) + (max(data_cum(:,1)) + min(data_cum(:,1)))/2;
...
% Begin optimization
% OPTIONS = optimset('MaxIter', 5000); %original options are written but not used
OPTIONS = optimset('MaxIter', 5000,'MaxFunEvals',10000); %zlx
quick = fminsearch(@(q)cost_function(q,data_cum,method),q0,OPTIONS); % options by Zlx
