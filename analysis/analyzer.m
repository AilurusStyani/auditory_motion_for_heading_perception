...
%     end
    ...
    disp([double('被试') subName double('在') dateNum double('测试中')]);
    disp([double('在汇报的') num2str(answerTrialNum) double('试次中，数字 | 出现频次')]);
    disp(AnsStats(:,1:2));
    
    reportedTrialNum = sum(~isnan(data.attentionRepAns(:,1)));
    AnsStats = tabulate(data.attentionRepAns(:,1));
    disp([double('在展示的') num2str(reportedTrialNum) double('试次中，数字 | 出现频次')]);
    disp(AnsStats(:,1:2));
    ...
    disp([double('数字选择的正确率是：')  num2str(sum(attCR)/length(attCR)) double('，漏失率是') num2str(sum(missR)/length(missR))]);
